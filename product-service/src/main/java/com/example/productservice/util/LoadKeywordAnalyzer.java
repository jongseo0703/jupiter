...
public class LoadKeywordAnalyzer {
    ...
    private void loadBodyKeywords() {
        bodyKeywords.put("부드러워요", Arrays.asList("부드", "목넘김 좋다","순하다", "매끄", "술술","마일드","편안","매끈","크리미","여운"));
        bodyKeywords.put("깔끔해요", Arrays.asList("무난","평범","적당","깔끔", "담백", "청량","라이트","경쾌","가뿐","맑다","밸런스"));
        bodyKeywords.put("묵직해요", Arrays.asList("거칠", "자극적","풍부","묵직","진하다","강함", "진한","무거움","숙성감","무게감"));
    ...
    private void loadValueKeywords() {
        valueKeywords.put("가성비가 좋아요", Arrays.asList("가성비", "저렴", "합리적", "부담 없는", "재구매", "합리적인","낮음"));
    ...
    private void loadSatisfactionKeywords() {
        satisfactionKeywords.put("추천해요", Arrays.asList("만족", "좋다", "또 마시고 싶다", "추천", "선물", "포장상태 좋은",
                "기대 이상", "입문자 추천", "재구매", "최고","맘에드네요","굳","기대","맛있","짱","편리","좋았","맛있다"));
    ...
    private Map<String, Integer> analyzeCategoryWithMorpheme(String comment, Map<String, List<String>> keywordMap) {
        ...
        for (Token token : tokens) {
            if (STOPWORDS.contains(token.getMorph())) continue; // 감탄사 제외
