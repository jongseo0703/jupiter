...
/**
 * 상품목록페이지 크롤링 서비스
 * 1단계: 단일 스레드로 상품 목록 페이지 크롤링
 * - 상품명, 브랜드, 이미지, 용량, 도수, 상세페이지 링크 추출
 */
@Service
@Slf4j
...
public class ListPageService {
    ...
    private ProductDTO getProduct(Element item) {
        ...
        String cleanImageUrl = cleanImageUrl(imageUrl);
    ...
    /**
     * 이미지 URL에 사이즈 정의 부분 삭제하는 메서드
     * @param url 이미지 URL
     * @return 사이즈가 정의되지 않은 이미지 URL
     */
    public static String cleanImageUrl(String url) {
        if (url == null) return null;

        // shrink=숫자:숫자 제거 (앞뒤 ? 또는 & 포함)
        String result = url.replaceAll("([?&])shrink=\\d+:\\d+(&)?", "$1");

        // ?& → ? 로 정리
        result = result.replace("?&", "?");

        // _v= → v= 로 변경
        result = result.replace("_v=", "v=");
